...
public class CharPanelViewer {
    ...
    CharPanelViewer(Panel panel, List<Character> characters) {
    ...
    private static void refreshLabels(Character character){
        for (Label label: panel.getCharStatsLabels()) {
            ...
            panel.getActionPointsLabels().forEach(aPLabel -> aPLabel.setVisible(Battle.isTurnMode()));
        }
    }
    ...
    private static void refreshPortrait(Character character) {
        panel.getCharPictBackgroundRect().setFill(character.getColor());
